...
class CompletedArticlesTracker:
    def __init__(self, tracker_file='./output/final_corpora/German/completed_articles.json'):
        """
...
with open('pipeline/wiki_articles_de.json', 'r', encoding='utf-8') as file:
    de_articles = json.load(file)


tracker = CompletedArticlesTracker()

# Process each domain and its articles
for domain, articles in de_articles.items():
    for article_title, url in articles.items():
        ...
        article_text = get_article_text(_title, 'de')

        if article_text == '':
            # Log the missing article
            log_missing_article(
                    './output/final_corpora/German/missing_articles_de.log',
        ...
        print(summary)
        print()
        meeting_planner = MeetingPlanner(client=CLIENT, model_id=MODEL_NAME, article=article_text, article_title=article_title, summary=summary, meeting_type=selected_meeting_type)
        meeting_plan, tags, personas = meeting_planner.plan_meeting(language=LANGUAGE)
        # print("-" * 20)
        print(meeting_plan)

        meeting_generator = MeetingGenerator(CLIENT, MODEL_NAME, personas, meeting_plan, article_text, article_title, selected_meeting_type, domain, language=LANGUAGE)
