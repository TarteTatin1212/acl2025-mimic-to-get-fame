...
for domain, articles in eng_articles.items():
    for article_title, url in articles.items():
        ...
        # meeting_lang = input("In which language do you want the meeting to be conducted? (Hit Enter/Return to continue in English) ")

        # LANGUAGE = 'English' if meeting_lang == '' else meeting_lang
        LANGUAGE = 'English'
        ...
        summary = transform_to_meeting_summary(article_title=article_title, article=article_text, client=CLIENT, model_id=MODEL_NAME, meeting_type=selected_meeting_type, language=LANGUAGE)
        # print("-" * 20)
        print(summary)
        print()
        ...
        meeting_generator = MeetingGenerator(CLIENT, MODEL_NAME, personas, meeting_plan, article_text, article_title, selected_meeting_type, domain)
